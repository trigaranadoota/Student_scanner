...
import { doc, getDoc, setDoc, onSnapshot, updateDoc, collection, query, where, orderBy, limit } from "firebase/firestore";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-brand-dark/60 backdrop-blur-md p-4">
      ...
      >
        ...
        <div className="p-10">
          ...
          <div id="reader" className="w-full overflow-hidden rounded-[2rem] bg-stone-50 border-2 border-dashed border-stone-200"></div>
  ...
  <header className="p-4 md:p-8 flex flex-col md:flex-row justify-between items-start md:items-end border-b border-brand-dark/5 bg-brand-bg/50 backdrop-blur-sm sticky top-0 z-40">
    ...
    <div className="flex items-center gap-4 mt-4 md:mt-0 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 border-brand-dark/5 pt-4 md:pt-0">
      <div className="text-left md:text-right">
        <p className="text-sm font-bold uppercase tracking-tight">{user.displayName || user.phoneNumber || "Student"}</p>
        ...
        <div key={entry.id} className="flex justify-between items-center border-b border-stone-100 pb-3 group">
          <div className="flex flex-col">
            ...
            <span className="text-[10px] text-stone-400 font-bold uppercase tracking-wider">
              {new Date(entry.timestamp?.toDate ? entry.timestamp.toDate() : entry.timestamp).toLocaleDateString("en-IN", { day: 'numeric', month: 'short' })} 
    ...
    {coupons.filter(c => !c.used).map((coupon) => (
      <div key={coupon.id} className="flex-shrink-0 w-64 p-6 border-2 border-brand-dark rounded-3xl bg-brand-cream relative overflow-hidden snap-start">
        ...
        <p className="mt-3 text-[9px] text-brand-green font-bold uppercase tracking-tighter">
          Expires: {new Date(coupon.expiry?.toDate ? coupon.expiry.toDate() : coupon.expiry).toLocaleDateString()}
        </p>
      </div>
    ))}
...
export default function App() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (u) => {
      ...
      if (u) {
        ...
        const ledgerQuery = query(collection(db, "ledger"), where("userId", "==", u.uid), orderBy("timestamp", "desc"), limit(10));
        const ledgerUnsub = onSnapshot(ledgerQuery, (snap) => {
          setLedger(snap.docs.map(d => ({ id: d.id, ...d.data() } as LedgerEntry)));
        });

        const couponQuery = query(collection(db, "coupons"), where("userId", "==", u.uid), orderBy("createdAt", "desc"));
        const couponUnsub = onSnapshot(couponQuery, (snap) => {
  ...
  const handleRedeem = async (tier: number) => {
    ...
    try {
      ...
      const data = await res.json();
      if (data.success) {
        // No need for state change, listener handles it
      } else {
        ...
        >
          ...
          <div className="space-y-4 w-full">
            <button 
              ...
              className="w-full flex items-center justify-center gap-3 bg-brand-dark hover:bg-black text-white font-black uppercase italic tracking-tight py-4 rounded-3xl transition-all hover:scale-[1.02] active:scale-[0.98]"
            >
              <img src="https://www.gstatic.com/firebasejs/ui/2.0.0/images/auth/google.svg" className="w-5 invert" alt="Google" />
              Sign in with Google
            </button>
            
            <div className="relative py-4 flex items-center">
              <div className="flex-1 border-t border-brand-dark/5"></div>
              <span className="mx-4 text-[10px] font-black uppercase tracking-widest text-stone-300">Auth Portal</span>
              <div className="flex-1 border-t border-brand-dark/5"></div>
            </div>
            ...
            {!confirmationResult ? (
              <form onSubmit={handlePhoneSubmit} className="space-y-3">
                <input 
                  ...
                  className="w-full px-6 py-4 bg-stone-50 border-2 border-stone-100 rounded-3xl focus:border-brand-green focus:outline-none transition-all font-bold text-sm tracking-tight text-center"
                />
                <button type="submit" id="phone-sign-in" className="w-full border-4 border-brand-dark hover:bg-brand-dark hover:text-white text-brand-dark font-black tracking-widest uppercase text-xs p-4 rounded-3xl transition-all">
                  Get Access Code
                </button>
              </form>
            ) : (
              <form onSubmit={handleOtpVerify} className="space-y-3">
                <input 
                  ...
                  className="w-full px-6 py-4 bg-stone-50 border-2 border-stone-100 rounded-3xl focus:border-brand-green focus:outline-none transition-all font-mono font-black text-xl tracking-[0.5em] text-center"
                />
                <button type="submit" className="w-full bg-brand-green hover:bg-brand-green/90 text-white font-black tracking-widest uppercase text-xs p-4 rounded-3xl transition-all">
                  Authorize Device
                </button>
              </form>
            )}
  ...
  return (
    <div className="min-h-screen bg-brand-bg flex flex-col">
      <Header user={user} profile={profile} onLogout={() => auth.signOut()} />
        ...
        <aside className="w-full md:w-1/3 p-6 md:p-8 border-b md:border-b-0 md:border-r border-brand-dark/5 flex flex-col justify-between bg-white overflow-y-auto">
          <div className="space-y-3 mb-12">
            ...
            <div className="flex items-baseline gap-2">
              <span className="text-8xl font-black tracking-tighter text-brand-dark">{profile.balance.toLocaleString()}</span>
              ...
              <div>
                ...
                <div className="space-y-4">
                  <button 
                    ...
                    disabled={profile.balance < 50 || isRedeeming}
                  ...
                  <button 
                    ...
                    disabled={profile.balance < 100 || isRedeeming}
                    className="w-full flex justify-between items-center p-4 border border-white/10 rounded-2xl hover:bg-white/5 transition-all disabled:opacity-30 group"
                  >
                    <div className="flex flex-col text-left">
